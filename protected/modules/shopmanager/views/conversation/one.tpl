...
{block name="content"}
    <a href="{Yii::app()->request->baseUrl}/shopmanager/conversation"><- Вернутся к списку</a> <br /> <br />
    Ваша переписка
    ...
    {foreach from=$messages item=message}
        {if $message->author_type=="user"}
            {include file="application.modules.shopmanager.views.conversation.message_user"}
        {else}
            {include file="application.modules.shopmanager.views.conversation.message_shop_user"}
        {/if}
    {/foreach}
